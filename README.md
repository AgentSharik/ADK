...
- **🧩 Расширяемость** — плагины (`Action`-класс → кнопка), CLI (`adk --find … --json`), серверный режим с PostgreSQL, portable, ru/en, 10 тем + свои цвета.
...
```bash
pyflakes adk tests
QT_QPA_PLATFORM=offscreen pytest -q          # 213 тестов
```
