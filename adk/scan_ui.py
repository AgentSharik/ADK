...
class FullScanWorker(BaseWorker):
    """Полный опрос парка: ПК (порциями по ширине пула сканера) → принтеры → характеристики.
...
def full_summary_text(s: dict) -> str:
    ...
    tail = (" «Кто за ПК» не ответил ни на одной включённой машине — WinRM и WMI, похоже, "
            "закрыты файрволом (порты 5985/135)." if s.get("user_missing_warn") else "")
