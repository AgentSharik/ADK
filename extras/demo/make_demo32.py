"""Видео-демонстрация ADK 3.8.0 (offscreen, заглушка LDAP) → /home/user/extras/videos/ADK_demo_31.mp4.

Ролик 31: новинки 3.8.0 — вопрос при старте («всё: ПК+принтеры+ПО» / «только ПК») с окном прогресса
и круговой диаграммой, кнопка «Область» в «ПО парка» и «Принтерах парка» (опрос одной организации
с колонкой «Пользователь»), свободный IP — ровный пробег по ячейкам по порядку. Плюс общий тур
возможностей на стендовой базе из 1500 ПК (как в ролике 30). В кадре — секундомер: время от нажатия
до появления строк и до статуса сети измеряется по настоящим часам. Титры — одна версия в начале и в конце.

Не входит в проект: лежит вне пакета проекта (папка demo/), в README/CHANGELOG/zip не упоминается.
Запуск: QT_QPA_PLATFORM=offscreen python /home/user/extras/demo/make_demo31.py
"""
import base64
import json
import os
...
ROOT = "/home/user"
OUT_DIR = "/home/user/extras/demo/frames32"
OUT_MP4 = "/home/user/extras/videos/ADK_demo_32.mp4"
FPS = 25
...
os.environ.setdefault("ADK_TESTS", "1")   # 3.8.0: стартовый вопрос сканирования не всплывает сам — отдельная сцена
...
from PyQt6.QtWidgets import QApplication, QLabel, QPushButton, QTableWidgetItem  # noqa: E402
...
from adk.setup_ui import ParkMaskDialog  # noqa: E402
...
from adk import fleetpoll as _fp  # noqa: E402
import adk.scan_ui as _scan_ui  # noqa: E402
from adk.scan_ui import FullScanDialog, StartupScanDialog, full_summary_text  # noqa: E402
from adk.widgets import OrgPickerDialog  # noqa: E402
...
# ============================================================ 1b. 3.9.x: имена ПК парка + полный опрос без окон
class _DemoFullScan(QThread):
    """Заглушка полного опроса: те же сигналы, что у настоящего FullScanWorker, но с быстрым сценарием."""
    ...
    def __init__(self, conn_factory, mode="full", parent=None):
    ...
    def run(self):
        import time as _t
        pcs, online = 1500, 610
        self.step_text.emit("Список рабочих станций из AD (LDAP)…")
        _t.sleep(0.6)
        self.step_text.emit("👥 Кто за ПК: спросим у самих машин и у журнала контроллера домена…")
        _t.sleep(0.8)
        self.plan.emit("pcs", pcs)
        self.plan.emit("printers", online)
        self.plan.emit("software", online)
        self.step_text.emit("Шаг 1 из 4 — ПК из домена: адреса, кто за каким ПК, последние входы…")
        for d in range(150, pcs + 1, 150):
            if self._stop:
                self.finished_full.emit({"pcs": d, "mode": "full", "stopped": True})
                return
            _t.sleep(0.55)
            self.unit.emit("pcs", d, "")
        self.step_text.emit("Шаг 2 из 4 — принтеры с ПК в сети…")
        for d in range(61, online + 1, 61):
            _t.sleep(0.5)
            self.unit.emit("printers", d, f"WS-{200 + d}")
        self.step_text.emit("Шаг 3 из 4 — программы с ПК в сети…")
        for d in range(61, online + 1, 61):
            _t.sleep(0.5)
            self.unit.emit("software", d, f"WS-{200 + d}")
        self.step_text.emit("Шаг 4 из 4 — характеристики (у кого данные старше 30 дней)…")
        self.plan.emit("specs", 214)
        for d in range(43, 215, 43):
            _t.sleep(0.4)
            self.unit.emit("specs", d, f"WS-{300 + d}")
        self.finished_full.emit({"pcs": pcs, "online": online, "printers": 214, "printers_pcs": 198,
                                 "sw": 5210, "sw_pcs": 193, "specs_pcs": 214, "mode": "full"})


cap("После выбора базы ADK сам спрашивает серии ПК парка — сколько найдено, видно сразу")
_rows = db.db_execute_with_retry(
    "SELECT computer_name FROM pc_inventory WHERE current_user != '' LIMIT 500", fetch="all")
_demo_names = [r[0] for r in _rows] or ["WS-101"]
_orig_save_section = config.settings.save_section
config.settings.save_section = lambda s, v: None          # демо не пишет в реальный конфиг
pm = ParkMaskDialog(w, _demo_names)
pm.show(); wait(300)
S2 = S + [pm]
snap(S2, 2)
hold(S2, 2.0)
...
def _row_edit(_pm, i):
    lay = _pm.rows_lay.itemAt(i)
    return lay.itemAt(0).widget() if lay and lay.count() else None

_e0 = _row_edit(pm, 0)
move_to(S2, _e0, 12)
type_text(S2, _e0, "WS-", 0.12)
hold(S2, 1.8)                                                # счётчик «найдено: N» пересчитался сам
press(S2, pm.btn_add); pm.add_row("PC-"); wait(250)
hold(S2, 1.8)                                                # вторая серия тоже посчиталась
move_to(S2, pm.btn_ok, 10)
press(S2, pm.btn_ok); pm.save(); wait(250)
pm.accept(); wait(200)
config.settings.save_section = _orig_save_section
S2 = S
hold(S, 1.6)

cap("Полный опрос стартует сам и идёт БЕЗ отдельных окон — счётчики в строке статуса")
_scan_ui.FullScanWorker, _orig_full_scan = _DemoFullScan, _scan_ui.FullScanWorker
_db_lease_orig = db.scan_lease_acquire
db.scan_lease_acquire = lambda owner, minutes=None: (True, owner, "")
w.start_full_scan()
wait(400)
snap(S, 2)
while w.full_worker and w.full_worker.isRunning():
    wait(120); snap(S, sec(0.12))
wait(300)
hold(S, 3.0)
cap("«Стоп» рядом, если надо прервать; сводка — тем же текстом в строке статуса")
move_to(S, w.btn_fill_stop, 10)
hold(S, 1.6)
_scan_ui.FullScanWorker = _orig_full_scan
db.scan_lease_acquire = _db_lease_orig
hold(S, 1.8)
cap("Дальше базу обновляет лёгкий фоновый опрос по расписанию: DNS, сверка с базой, пинг 100 мс")
move_to(S, w.btn_scan, 12)
hold(S, 2.2)
...
# --- 3.8.0: «Область» — опрос только ПК выбранной организации
cap("«Область»: опрашиваем только ПК выбранной организации — кто какой принтер использует")
move_to(S, prd.btn_org, 12)
press(S, prd.btn_org)
_orig_companies = ad.get_all_attribute_values
ad.get_all_attribute_values = lambda conn, attr: ["ООО «Пример»", "Северный филиал", "Южный филиал"]
op2 = OrgPickerDialog(w.get_conn, w)
op2.show(); tg._wait(lambda: op2.list.count() == 3, app, 3000); wait(200)
S2 = S + [op2]
snap(S2, 2)
hold(S2, 1.4)
move_to(S2, op2.filter, 10)
type_text(S2, op2.filter, "Север", 0.1); wait(200)
hold(S2, 1.2)
op2.list.setCurrentRow(0); wait(150)
move_to(S2, op2.btn_ok, 10)
press(S2, op2.btn_ok)
op2._pick(); wait(200)
S2 = S
_ORG_HOSTS = ["WS-101", "WS-137", "WS-212", "WS-305", "WS-341", "WS-402"]
_ORG_USERS = {"WS-101": "Иванов Иван Петрович", "WS-137": "Громова Елена Викторовна", "WS-212": "Соколов Дмитрий Олегович",
              "WS-305": "Лебедева Ольга Николаевна", "WS-341": "Морозов Максим Игоревич", "WS-402": "Ефимова Ирина Павловна"}
_ORG_PRINTERS = {
    "WS-101": {"printers": [{"name": "HP LaserJet M404dn", "port": "IP_10.0.12.50", "kind": "network", "ip": "10.0.12.50"},
                            {"name": "Canon LBP6030", "port": "USB001", "kind": "usb", "ip": ""}]},
    "WS-137": {"printers": [{"name": "Kyocera ECOSYS P3145dn", "port": "10.0.12.51_1", "kind": "network", "ip": "10.0.12.51"}]},
    "WS-212": {"printers": [{"name": "HP LaserJet M404dn", "port": "IP_10.0.12.50", "kind": "network", "ip": "10.0.12.50"},
                            {"name": "Epson L3150", "port": "USB002", "kind": "usb", "ip": ""}]},
    "WS-305": {"printers": [{"name": "Kyocera на PRINTSRV", "port": "\\\\PRINTSRV\\KYO-2F", "kind": "shared", "ip": ""}]},
    "WS-341": {"printers": []},
    "WS-402": {"printers": [{"name": "Kyocera ECOSYS P3145dn", "port": "10.0.12.51_1", "kind": "network", "ip": "10.0.12.51"}]},
}


class _DemoFleetPoll(QThread):
    """Заглушка опроса парка: сигналы как у FleetPollWorker, темп — 0,55 с на ПК."""
    progress = pyqtSignal(int, int, str)
    finished_poll = pyqtSignal(dict)
    error = pyqtSignal(str)

    def __init__(self, hosts, fn, parent=None):
        super().__init__(parent)
    ...
    def run(self):
        ...
        for i, h in enumerate(self.hosts, 1):
            ...
            _t.sleep(0.55)
            out[h] = self.fn(h)
            self.progress.emit(i, len(self.hosts), h)
        self.finished_poll.emit(out)


_orig_fp_worker = _fp.FleetPollWorker
_fp.FleetPollWorker = _DemoFleetPoll
_orig_printers_live = _fp.printers_live
_fp.printers_live = lambda host: _ORG_PRINTERS.get(host, {"printers": []})
prd._org_users, prd._org_name = dict(_ORG_USERS), op2.choice
prd._start_live(_ORG_HOSTS)
while prd.worker and prd.worker.isRunning():
    wait(120); snap(S, sec(0.12))
wait(300)
hold(S, 2.6)
move_to(S, prd.text, 10)
type_text(S, prd.text, "Громова", 0.1); wait(300)
hold(S, 1.8)
prd.text.clear(); wait(200)
_fp.FleetPollWorker = _orig_fp_worker
_fp.printers_live = _orig_printers_live
ad.get_all_attribute_values = _orig_companies
S = hide_dialog(S, prd)
...
# ============================================================ 11b. 3.8.0: ПО парка — «Область»
cap("ПО парка — «Область»: опрос только организации, видно, какой пользователь какое ПО использует")
sd2 = SoftwareDialog("", w, w)
S = show_dialog(S, sd2)
hold(S, 1.8)
move_to(S, sd2.btn_fleet_org, 12)
press(S, sd2.btn_fleet_org)
ad.get_all_attribute_values = lambda conn, attr: ["ООО «Пример»", "Северный филиал", "Южный филиал"]
op3 = OrgPickerDialog(w.get_conn, w)
op3.show(); tg._wait(lambda: op3.list.count() == 3, app, 3000); wait(200)
S2 = S + [op3]
snap(S2, 2)
hold(S2, 1.2)
op3.list.setCurrentRow(1); wait(150)      # «Северный филиал»
move_to(S2, op3.btn_ok, 10)
press(S2, op3.btn_ok)
op3._pick(); wait(200)
S2 = S
_ORG_SOFT = {}
_SOFT_SETS = [
    ("1С:Предприятие 8.3", "8.3.24.1314"), ("Google Chrome", "128.0"), ("Kaspersky Endpoint Security", "12.6"),
    ("Microsoft Office LTSC 2021", "2021"), ("7-Zip", "24.08"),
]
for _i, _h in enumerate(["WS-101", "WS-137", "WS-212", "WS-305", "WS-341", "WS-402"]):
    _ORG_SOFT[_h] = {"software": [{"name": n, "version": v, "publisher": "", "installed": ""}
                                  for n, v in _SOFT_SETS[_i % 2:] + _SOFT_SETS[:_i % 2 + 1]]}
sd2._org_users, sd2._org_name = dict(_ORG_USERS), op3.choice
sd2.fleet.setHorizontalHeaderLabels(["Пользователь", "ПК", "Программа", "Версия"])
sd2.fleet_lbl.setText(f"🏢 «{op3.choice}»: опрашиваю {len(_ORG_HOSTS)} ПК организации…")
_orig_soft_live = _fp.software_live
_fp.software_live = lambda host: _ORG_SOFT.get(host, {"error": "не в сети"})
sd2._start_fleet_poll(_ORG_HOSTS)
while sd2.fleet_worker and sd2.fleet_worker.isRunning():
    wait(120); snap(S, sec(0.12))
wait(300)
hold(S, 2.6)
move_to(S, sd2.q, 10)
type_text(S, sd2.q, "1С", 0.12); sd2.search_fleet(); wait(250)
hold(S, 2.4)
sd2.q.clear(); sd2.search_fleet(); wait(200)
hold(S, 1.6)
_fp.FleetPollWorker = _orig_fp_worker
_fp.software_live = _orig_soft_live
ad.get_all_attribute_values = _orig_companies
S = hide_dialog(S, sd2)
...
# ============================================================ 13. Свободный IP
cap("Свободный IP: пробег по ячейкам строго по порядку — статус после проверки, рамка — текущая")
...
# 3.8.0: настоящий поиск — воркер сам закрашивает карту по порядку; текущая ячейка в рамке,
# результат появляется после проверки. Проверки в демо paced: ответ каждой «доходит» по очереди.
class _NoPtrFut:
    def cancel(self):
        pass


_alive31_prev = netutils.is_host_alive
_ptr_start_prev, _ptr_wait_prev = _wk._ptr_start, _wk._ptr_wait


_freeip_base = {"v": 60}                                     # стартовый адрес текущего поиска (для темпа)


def _alive31(ip, timeout=1.0):
    h = int(ip.rsplit(".", 1)[1])
    _time.sleep(0.3 + 0.5 * max(0, h - _freeip_base["v"]))  # ответы приходят по очереди — ровный пробег
    return h % 17 != 5                                       # свободны .73, .90, .107…


netutils.is_host_alive = _alive31
_wk._ptr_start = lambda ip: _NoPtrFut()
_wk._ptr_wait = lambda fut, timeout: False
FreeIPWorker._dhcp_cache.clear()
fd.search()                                                  # «Найти» уже нажат — идёт настоящий поиск
while fd.worker and fd.worker.isRunning():
    wait(110); snap(S, sec(0.11))
wait(300)
hold(S, 3.0)
move_to(S, fd.lbl_ip, 12)
hold(S, 1.6)
cap("«Следующий»: новый поиск начнётся после найденного адреса — без ожидания хвоста проверок")
move_to(S, fd.btn_next, 10)
press(S, fd.btn_next)
_freeip_base["v"] = int(fd.found.rsplit(".", 1)[1]) + 1
fd.next_()
while fd.worker and fd.worker.isRunning():
    wait(110); snap(S, sec(0.11))
wait(250)
hold(S, 2.4)
netutils.is_host_alive = _alive31_prev
_wk._ptr_start, _wk._ptr_wait = _ptr_start_prev, _ptr_wait_prev
S = hide_dialog(S, fd)
...
if atd.table.rowCount() > 1:
    ...
    hold(S, 1.8)
cap("✅ «Прочитать всё» — скрыть всё показанное разом; сводка появится снова, когда что-то изменится")
move_to(S, atd.btn_read_all, 10)
press(S, atd.btn_read_all); atd.read_all(); wait(300)
hold(S, 2.0)
S = hide_dialog(S, atd)
...
b_design = [b for b in w.findChildren(QPushButton) if "Настройки" in b.text()][0]   # 3.6.x: «Дизайн» → «⚙️ Настройки»
...
snap(S, 3)
cap("Темы — двумя группами: тёмные и светлые; сброс к умолчанию — одной кнопкой")
_btn_theme_reset = [b for b in dd.findChildren(QPushButton) if "Сбросить к теме" in b.text()]
if _btn_theme_reset:
    move_to(S, _btn_theme_reset[0], 10)
    hold(S, 1.0)
    press(S, _btn_theme_reset[0]); dd.preset("dark"); wait(400)
    snap(S, 2)
hold(S, 1.2)
hold(S, 1.4)
...
hold(S, 2.2)
cap("В инспекторе — кто за ПК и сводка характеристик: ОС, процессор, память, диски")
move_to(S, w.lbl_specs, 12)
hold(S, 2.4)
w.search_input.clear(); w.on_text_changed(""); wait(300)
_cursor[:] = [W // 2, H // 2 + 120]
hold(S, 2.4)
