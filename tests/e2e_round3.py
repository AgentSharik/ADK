...
check("printer: Kyocera → только сам принтер (3.6.3; владельцы — в инспекторе)", comps == [""], str(comps))
pcs = sorted(x["comp"] for x in (got[0][0].get("printer") or {}).get("pcs", [])) if got else None
check("инспектор принтера знает владельцев: WS-102 (kuznetsova) и WS-133 (свободный ПК)", pcs == ["WS-102", "WS-133"], str(pcs))
...
check("инспектор: 2 бейджа (сетевой с IP и «по умолчанию», USB), без Microsoft PDF", [b.text() for b in badges] == ["HP LaserJet M404dn · 10.0.2.50 · по умолчанию", "Canon LBP6030"], str([b.text() for b in badges]))
check("бейджи: одна иконка — вид подключения", all(not b.icon().isNull() for b in badges))
...
check("клик по USB-бейджу → только сам принтер (подключено 1 ПК — в инспекторе)", "подключено ПК — 1" in w.lbl_status.text() and sorted(w.table.item(r, 0).text() for r in range(w.table.rowCount())) == ["—"], w.lbl_status.text())
w.search_input.setText("printer: Kyocera"); w.start_search()
tg._wait(lambda: w.table.rowCount() == 1 and "Kyocera" in (w.table.item(0, 1).text() or ""), app, 3000)   # только принтер (3.6.3)
w.select_row(0)
check("принтер: инспектор показывает владельцев (WS-102 + WS-133) и бейдж Kyocera", "Kyocera" in w.lbl_fio.text() and w.printer_pcs.rowCount() == 2, w.lbl_fio.text())
