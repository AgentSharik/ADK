...
становится кнопкой в инспекторе и пунктом контекстного меню. Файлы с `_` в начале имени игнорируются
(так поставляется пример `_example_user_profile.py`). Ошибка в плагине пишется в лог и не ломает остальные.
