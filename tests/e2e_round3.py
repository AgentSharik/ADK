...
check("клик по сетевому бейджу → поиск по IP → только сам принтер (владельцы — в инспекторе)", w.search_input.text() == "10.0.2.50" and logins == ["🖨️"], str(logins))
...
w.search_input.setText("иванов"); w.start_search()
tg._wait(lambda: w.table.rowCount() > 0 and w.table.item(0, 0).text() != "🖨️", app, 3000)
w.select_row(next(r for r in range(w.table.rowCount()) if w.table.item(r, 0).text() == "ivanov"))
...
check("клик по USB-бейджу → подключён 1 ПК, 1 пользователь", "подключено ПК — 1" in w.lbl_status.text() and sorted(w.table.item(r, 0).text() for r in range(w.table.rowCount())) == ["ivanov", "🖨️"], w.lbl_status.text())
