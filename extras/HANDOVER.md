...
Актуально на 2026-09-22. Версия проекта **3.9.7**, тестов **411**, e2e 46 + 83 + 45 + 41, стенд `tests/bench/` (1500 ПК), видео-ролик **32** (730 с). Раздел О — обязательный регламент работы.
...
- Юзер (скриншот страницы релиза): «приведи это к стандарту других изменений версии». Исправлено: имя релиза было «ADK v3.9.6» без описания → теперь с описанием, как у 3.9.4/3.9.5; тело — «## 3.9.6 — …», секции «### 🐞 Исправлено» (1–3) и «### 🛠 Улучшено (движок «Доменного Радара»)» (4–5, раньше была дыра в нумерации 5–6 после 1–3), «### 🧪 Тесты +4 … Всего 411». CHANGELOG в репо синхронно поправлен (заголовок секции, нумерация, формат тестов); коммит на main БЕЗ переноса тега (косметика, пересборка не нужна).
