...
Актуально на 2026-09-20. Версия проекта **3.9.1**, тестов **394**, e2e 46 + 83 + 45 + 41, стенд `tests/bench/` (1500 ПК), видео-ролик **31** (709 с). Раздел О — обязательный регламент работы.
...
### Полный перевод EN — статичные строки закрыты
- Все 13 интерфейсных модулей (main_window, dialogs, health_ui, fleet, freeip_ui, setup_ui, tools, inventory_ui, pingui, widgets, notify, attention_ui, scan_ui) полностью обёрнуты в tr(): детектор (QPushButton/QLabel/setToolTip/setText/заголовки/вкладки) находит 0 необёрнутых кириллических литералов. Словарь EN +~250 записей за два захода.
- tests/test_i18n_progress.py: три проверки — все модули обёрнуты; все tr()-литералы есть в словаре; smoke переключения языка (en → перевод, отсутствующая строка → русский фолбэк).
- ГРАБЛИ автообёртки: (1) конкатенации литералов «QLabel("A" "B")» — обёртка первой части ломает синтаксис; фикспоинт-регекс втягивает хвосты внутрь tr(); (2) хвост-f-строка («tr("A") f"B{x}"») — решается «tr("A") + f"B{x}"» (переводится статичная часть); (3) в многочастных (3+) нужно поглощать до стабильности.
- ОСТАЛОСЬ для полного EN: f-строки с кириллицей (прогресс сканера «⚡ Опрос {n} ПК…» и т.п.) — переводить разбиением на tr("…") + переменная; сообщения MessageBox вне tr(); длинные тексты справки плагинов.
- Тестов 397 (+3 i18n). Видео 32 сдано ранее.
