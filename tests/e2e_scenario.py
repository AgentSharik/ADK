...
d3=FreeIPDialog(w); d3.show(); d3.prefix.setText("bad"); d3.search(); tg._wait(lambda: "⚠️" in d3.status.text(), app, 3000)
check("свободный IP: валидация префикса", "⚠️" in d3.status.text()); d3.close()
...
d6=DesignSettingsDialog(w,w); d6.show(); d6.preset("sunset"); check("смена темы применяется", config.settings.design["accent_color"]=="#fb7185"); d6.close()
...
w.search_input.setText("кто-то"); w.start_search(); tg._wait(lambda: "⚠️" in w.lbl_status.text(), app, 3000)
check("недоступный AD → сообщение, а не вечный «Поиск…»", "⚠️" in w.lbl_status.text(), w.lbl_status.text())
