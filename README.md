...
```bash
pyflakes adk tests
QT_QPA_PLATFORM=offscreen pytest -q          # 298 тестов
```
...
- **3.5 (актуальная):** автоопределение параметров домена AD, мастер «где база?» при первом запуске с автонаполнением, опрос парка без базы, справка по роли после входа, менеджер плагинов с шаблоном, 298 тестов.
