...
└── extras/              НЕ проект: не в zip, не в README/CHANGELOG
    ...
    ├── videos/          РОВНО ОДИН ролик: ADK_demo_24.mp4
    ├── demo/            РОВНО ОДИН сценарий make_demo24.py + make_music.py
...
**Обязательные сцены** (все есть в `make_demo24.py`, в этом порядке): вход (показать/скрыть пароль, «запомнить»);
...
экран, RoleInfoDialog, инспектор, карточка только для чтения); оформление (все 10 тем, свой акцент). Сцены
...
1. `cp extras/demo/make_demo24.py extras/demo/make_demo25.py`; заменить в нём `frames24`→`frames25`, `ADK_demo_24`→`ADK_demo_25`,
...
текста. Тестов 234.
**3.5.2** (текущая) — карточка/инспектор/таблица сразу отражают смену пароля и снятие блокировки
...
(6 тёмных / 4 светлые). Тестов 236. Видео — `extras/videos/ADK_demo_24.mp4` (сценарий `extras/demo/make_demo24.py`,
с субтитрами, одна версия 3.5.2 в титрах, длительность — в заметке ниже).

Длительность роликов (для правила «не короче предыдущего»): 21 — 8:51 (531 с); 23 — 9:22 (562 с); **24 — 9:36 (576 с)** (файл ~20 МБ).
Темп статичных пауз задаётся константой `HOLD_SCALE` в `make_demo24.py` (сейчас 1.25) — если новый ролик выходит короче,
проще всего поднять её, а не резать сцены; живые сегменты пинга — `live(S, 14.0)` / `live(S, 12.0)`.

Открытые задачи: **полная переработка окна «Пинг»** (автор ещё не описал желаемое); при следующем видео — номер 25.
