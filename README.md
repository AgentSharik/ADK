...
Первый запуск создаёт `Documents\ADK\config.ini` — заполните LDAP-параметры (образец `config.example.ini`) и запустите снова. ADK спросит, где лежит (или будет лежать) база: существующую подхватит, новую создаст и сам наполнит парком и принтерами. Ставьте ADK на **один** ПК, остальным — ярлык: база будет одна.
...
Подробно (зависимости, PyCharm, откуда брать данные домена, `use_ssl`, роли, серверный режим, плагины): **[docs/INSTALL.md](docs/INSTALL.md)**.
...
- База — один файл SQLite; для отдела ADK и база живут на одном ПК/сервере, у остальных ярлык (`adk --serve` — сканер по расписанию без окна).
...
- **3.6 (актуальная):** база без WAL (переносится на сервер), сторож зависшего запроса, резервные копии каждые 6 ч, один сканер на общую базу; SQLite — единственный движок (PostgreSQL-адаптер удалён).
- **3.5:** автоопределение параметров домена AD, мастер «где база?» при первом запуске с автонаполнением, опрос парка без базы, справка по роли после входа, менеджер плагинов с шаблоном, 306 тестов.
