...
class BulkOperationsDialog(FramelessDialog):
    ...
    def __init__(self, users: list[dict], app, parent=None):
        ...
        top.addWidget(QLabel(tr("Операция:")))
        top.addWidget(self.op, 1)
        self.body.addLayout(top)

        self.group_row = QHBoxLayout()
        self.group_filter = QLineEdit()
        self.group_filter.setPlaceholderText(tr("Фильтр групп…"))
        ...
        self.must_change = QCheckBox(tr("Потребовать смену пароля при входе"))
        ...
        self.btn_copy = QPushButton(tr("📋 Скопировать пароли"))
        self.btn_copy.setVisible(False)
        self.btn_copy.clicked.connect(self.copy_passwords)
        btns.addWidget(self.btn_copy)
        self.btn_run = QPushButton(tr("▶ Выполнить"))
    ...
    def run_op(self):
        ...
        self.status.setText(tr("⏳ Выполняется…"))
...
class NotesDialog(FramelessDialog):
    ...
    def __init__(self, subject: str, kind: str, app, parent=None, title: str = ""):
        ...
        self.edit.setPlaceholderText(tr("Новая заметка… (Ctrl+Enter — сохранить)"))
        self.edit.setMaximumHeight(90)
        self.body.addWidget(self.edit)
        btns = QHBoxLayout()
        self.btn_del = QPushButton(tr("🗑 Удалить выбранную"))
        self.btn_del.clicked.connect(self.delete_selected)
        btns.addWidget(self.btn_del)
        btns.addStretch()
        self.btn_add = QPushButton(tr("💾 Сохранить заметку"))
...
class GroupCompareDialog(FramelessDialog):
    ...
    def __init__(self, target_entry, app, parent=None):
        ...
        top = QHBoxLayout()
        self.ref = QLineEdit()
        self.ref.setPlaceholderText(tr("Логин эталонного пользователя (например, petrov)"))
        self.ref.returnPressed.connect(self.load_ref)
        btn = QPushButton(tr("Сравнить"))
        btn.setObjectName("btnPrimary")
        btn.clicked.connect(self.load_ref)
        top.addWidget(QLabel(tr("Эталон:")))
        ...
        self.status = QLabel("")
        btns.addWidget(self.status, 1)
        self.btn_add = QPushButton(tr("➕ Добавить выбранные"))
        self.btn_add.setObjectName("btnSuccess")
        self.btn_add.clicked.connect(lambda: self.apply(self.missing, ad.MODIFY_ADD))
        self.btn_rm = QPushButton(tr("➖ Удалить выбранные"))
