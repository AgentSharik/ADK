...
Актуально на 2026-09-18. Версия проекта **3.6.2**, тестов **306**, e2e 46 + 83 + 45 + 41, стенд `tests/bench/` (1500 ПК). Раздел О — обязательный регламент работы.
...
**3.6.2** — только документация: формулировки про WAL нейтральные («не поддерживается на сетевых папках»), история git переписана `filter-repo --replace-text`, тексты релизов 3.6.0/3.6.1 отредактированы. **Правило:** о режиме журнала и базе писать только нейтрально, без слов о поломках/блокировках.
