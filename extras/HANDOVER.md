...
Актуально на 2026-09-24. Версия проекта **3.9.15**, тестов **421**, e2e 46 + 83 + 45 + 41, стенд `tests/bench/` (1500 ПК), видео-ролик **32** (730 с). Раздел О — обязательный регламент работы.
...
### Партия 67: DCOM был мёртв на PS 5.1 (3.9.15)
- Юзер прогнал fast_who_is_online.ps1 (standalone-скрипт из партии 66): у всех досягаемых машин «CIM: Не удается найти параметр, соответствующий имени параметра \"Protocol\"». ДИАГНОЗ: `New-CimSession -Protocol Dcom` валиден ТОЛЬКО в PowerShell 7; в Windows PowerShell 5.1 (им работает exe) у New-CimSession нет -Protocol — нужен `New-CimSessionOption -Protocol Dcom` → `-SessionOption`. ОШИБКА МОЯ: проверял скрипт на pwsh 7, где конструкция валидна.
- ВАЖНО: тот же паттерн был в _PS_SCANNER (с 3.9.9!) → ветка DCOM дозапроса в приложении молчала все 3.9.9-3.9.14. У юзера 5985 закрыт везде, 135 открыт → дозапрос терял юзеров ровно на этом (вероятная часть «нашёл только 38»). Также и standalone-скрипт был с этим багом.
- Сделано: workers.py — `$sess = New-CimSession -ComputerName $pc -SessionOption (New-CimSessionOption -Protocol Dcom)`; предпровер $port — `$open = $c.ConnectAsync(...).Wait(400) -and $c.Connected` (RST-отказ завершает задачу быстро и Wait()=true → ложное «открыт»); fast_who_is_online.ps1 — тот же фикс.
- ГРАБЛИ: pwsh 7 для проверки НЕ ловит несовместимость с 5.1 (в 7 New-CimSession -Protocol валиден). Правильная проверка: аудит PS-конструкций по списку 7-only (-Protocol у New-CimSession, Test-Connection -TimeoutSeconds, тернарный ?, ??, ForEach -Parallel, -AsHashtable) + парсер-проверка извлечённых скриптов с подстановкой плейсхолдеров (голый __POOL__ в выражении роняет парсер — подставлять значения!). Обе проверки сделаны в этой партии; тест test_ps_scanner_powershell51_compatible закрепляет.
- Тест: старый 3.9.9-тест проверял битый паттерн — обновлён на SessionOption-форму.
- 421 passed + 2 skipped. Тег v3.9.15.

### Партия 66: все КД домена + окно времени (3.9.14)
- Юзер: «он всё равно нашёл не всех, только 38 ПК — может, есть ещё способы найти все ПК?». Диагноз-гипотеза: (1) в домене несколько КД — события входа пишутся на ТОТ КД, который аутентифицировал вход, один настроенный dc_host видел лишь часть входов (главный подозреваемый); (2) аудит входов (4768/4776) может быть выключен вовсе; (3) окно 50к событий в шумном домене — часы.
- Сделано в workers.py: `_DC_LOGON_PS` — список КД (настроенный первым + SRV `_ldap._tcp.dc._msdcs.<домен>` через Resolve-DnsName, per-DC try/catch, `$okDc/$totalEv/$err0`); FilterHashtable с `StartTime = Now.AddHours(-__HOURS__)`; строка `DCINFO ok/total events:...; err:...` перед JSON; все КД молчат → throw no DC responded. `_domain_for_srv()` — домен из search_base (regex dc=) иначе upn_suffix. `dc_logon_events(dc_host, max_events=None, timeout=600, progress=None)`: подстановки __DOMAIN__/__HOURS__, DCINFO-строки вырезаются из stdout, регекс-разбор → progress-сообщение «контроллеров ответило X из Y, событий Z». `deep_scan_dc_first`: передаёт say в dc_logon_events; pairs==[] без исключения → подсказка про выключенный аудит.
- config.py [AD]: `dc_logon_hours` (default 72; 0 = без окна) — defaults/шаблон/Settings. dc_logon_max_events остаётся потолком НА КД.
- Тесты: моки dc_logon_events в deep_scan-тестах расширены `progress=None`; новый тест SRV+окна (проверка через `if (72 -gt 0)`/`AddHours(-72)` — НЕ через "StartTime", слово есть в комментарии скрипта!) и тест подсказки про аудит.
- ГРАБЛИ: `"StartTime" not in script` и `"AddHours(-" not in script` — ложные срабатывания из-за комментария/подстановки текстом; проверять подставновку условия `if (N -gt 0)`.
- Юзеру также отвечено (без кода): утечка данных (что собираем/куда/риски) и зачем был таймаут 900 с (предохранитель от зависшего PS-процесса; теперь масштабируемый).
- 420 passed + 2 skipped. Тег v3.9.14.

### Партия 65: непрерывный пул, без порций (3.9.13)
- Юзер: «при подходе к концу партии (200) замедляется: 0-190 быстро, с 190 ползёт, после 200 снова быстро, на 390 снова». Диагноз: «хвост порции» — порция 200 = отдельный PS-процесс с пулом 64; в конце остаются «молчуны» (выключенные ждут DNS-таймаута, полуживые — таймаутов портов/протоколов; реестр в дозапросе таймаута не имеет), до 63/64 потоков простаивают, следующая порция не стартует. Пила каждые 200.
- Сделано: scan_ui — CHUNK убран, deep_scan_dc_first зовётся без chunk_size (один непрерывный список и в лёгкий проход, и в дозапрос; параметр chunk_size в workers оставлен для совместимости сигнатуры); workers._run_powershell — таймаут-предохранитель = min(7200, 300 + волны × (120 если wmi_user иначе 30)), волны = ceil(len/pool_eff), pool_eff вычисляется один раз и подставляется в __POOL__ (было 900 с фиксированно — парк одним куском мог не уложиться и упасть в Python-fallback без юзеров!).
- Список ПК и так идёт временным JSON-файлом (__INPUT__) — ограничений размера нет; отмена — psrun cancelled каждые 0,5 с, работает внутри одного большого запуска.
- Журнал КД и раньше запрашивался ОДИН раз на весь опрос (в deep_scan_dc_first, не на порцию) — не трогал.
- 418 passed + 2 skipped. Тег v3.9.13.

### Партия 64: журнал КД — NTLM 4776 + настраиваемый объём (3.9.12)
- Юзер: «счётчик говорит прошёл 399, а реально ~34, ещё и долго; может выгружать из ГПО каждого ПК кто вошёл — так быстрее?». Диагноз: похоже, файрвол/GPO режет WMI на большинстве машин — счётчик дозапроса «пролетает» (машины быстро отбрасываются предпровером портов без результата), хвост тянет реестр (OpenRemoteBaseKey без таймаута — известный остаточный риск). Курс подтверждён: журнал КД — основной источник «кто за ПК», ПК трогать минимум.
- Ответ про ГПО (дан юзеру): сама ГПО «кто вошёл» не хранит; доменный аудит (настраивается ГПО) пишет события на контроллеры домена — это и есть наш журнал КД. Второе прочтение — журналы логон-скриптов ГПО на шаре (= старые CSV, удалённые по приказу юзера) — упомянуты как опция только по явной просьбе.
- Сделано в adk/workers.py: `_DC_LOGON_PS` — `Id=@(4768,4776)`; для 4776 проверка `Status 0x0`, ключ `ws:ИМЯПК` (Workstation; домен/бэкслеш отрезаются PS-сплитом `'[\\/]'`), `ResultCode` — только для 4768; `dc_logon_events(dc_host, max_events=None, timeout=240)` — None → `settings.dc_logon_max_events`, ≤0 → RuntimeError «чтение журнала отключено»; `merge_dc_logons` — ws:-ключи матчятся по имени ПК (upper + rsplit по `\\` и `/` от доменного префикса), общий порядок событий сохранён (первое = самое свежее событие на ПК выигрывает), hosts-множество из результатов сканера; PTR — только остаток IP-ключей.
- adk/config.py: [AD] `dc_logon_max_events` (default 50000) — defaults-словарь, шаблон ad_sec, Settings-атрибут. Миграция допишет ключ в существующий config юзера (значения не перезаписывает).
- ГРАБЛИ: падение нового теста было из-за отсутствия отрезания домена в merge (ws:CORP\PC-0001), а НЕ из-за бэкслешей в тесте — в heredoc `\\` даёт 2 символа в файле = 1 в значении, это правильно.
- 417 passed + 2 skipped. Тег v3.9.12.

### Партия 63: живой прогресс опроса (3.9.11)
- Юзер (скриншот, OCR не читается): «показывает 200 из 400 — не понятно, сколько он сейчас делает». Заметка: дозапрос у юзера уже 400 машин (было 1277) — IP-сопоставление журнала КД из 3.9.10 сработало.
- Сделано: psrun.run(…, on_line=…) — потоковое чтение stdout (потоки-насосы stdout/stderr + queue, отмена/таймаут каждые 0,5 с сохранены); _PS_SCANNER — цикл ожидания задач (IsCompleted), печатает «PRG d/t» каждые 150 мс; _run_powershell(label/offset/grand) парсит PRG → self.progress.emit(f"{label} {offset+d} из {grand}"), PRG-строки вырезаются перед json.loads; probe_hosts(offset, grand) — label по wmi_user («⚡ Опрошено» / «👥 Дозапрос:»); deep_scan_dc_first.run_all ведёт накопительный base-офсет по порциям; сигнатура probe → (hosts, wmi_user, offset, grand) — обновлены все 3 вызова (run, scan_once, FullScanWorker._probe) и моки тестов.
- 416 passed + 2 skipped. Тег v3.9.11.

### Партия 62: журнал КД по IP, без PTR (3.9.10)
- Юзер: «всё равно ищет по 200, хотя в конфиге 64, и дозапрос ооочень долго». Диагноз: (1) «200 из 1277» — счётчик порций CHUNK=200, не пул; (2) 1277 ПК в дозапросе — merge_dc_logons матчил события КД только через обратный DNS (socket.gethostbyaddr), PTR в сети юзера нет → журнал КД почти никого не закрывал.
- merge_dc_logons: сперва точный матч ip события ↔ ActualIp из лёгкого прохода (ip_pc map, setdefault), PTR — только для несовпавших пар (unresolved). Тест: gethostbyaddr кидает AssertionError — не должен зваться.
- Статусы: «⚡ Опрос N ПК (DNS, ping, пул X)…», «Дозапрос у машин: D из T · параллельно X»; при недоступном журнале КД — ⚠️ + подсказка про dc_host (config.ini → [AD]) если он не задан.
- Юзеру объяснить: config читается из ADK\config.ini РЯДОМ С EXE (portable приоритетнее Документов); проверить, что стоит 3.9.9+ (в 3.9.8 дозапрос без предпровера и с безтаймаутным Get-WmiObject — отсюда «очень долго»).
- 415 passed + 2 skipped. Тег v3.9.10, релиз PATCHнут.
...
- build-exe.yml: Remove-Item dist\ADK перед Compress-Archive — smoke-прогон создавал папку ADK с config.ini, и она попадала в zip; юзер просил «пусть сам создаёт». Проверка создания config в CI осталась.
- Юзеру сообщить: если его config.ini содержит scan_pool_width = 200 (вписан 3.9.8) — поменять на 64 или удалить строку; миграция значения не перезаписывает.

### Партия 60: щадящий опрос + полное удаление CSV-папок и упоминаний прошлого продукта (3.9.8)
- Юзер: «папки invent_comp_dir/invent_compexit_dir и любое упоминание [прошлого продукта] удалить» + «200 запросов с одного ПК — страшно, есть другие методы узнать кто сидит?». Удалено: ключи config (defaults/шаблон/Settings), get_pc_info_from_csv (netutils), fallback архивов «Прочитать всё» на CSV, строки в config.example.ini, docs/INSTALL.md, extras/data/ADK/config.ini; справочник прошлого продукта удалён из воркспейса и .gitignore; CHANGELOG/HANDOVER вычищены от имени продукта.
- Щадящий опрос (deep_scan_dc_first в workers): (1) лёгкий проход всем парком (DNS+пинг, без WMI, pool=scan_pool_width); (2) ОДИН запрос dc_logon_events к журналу КД (4768) → merge_dc_logons (обратный DNS); (3) WMI-каскад ТОЛЬКО для включённых ПК без юзера (gaps); журнал КД недоступен/пуст → gaps=все включённые (старое поведение). Подключено: PCScannerWorker.run, scan_once, FullScanWorker (chunk_size=CHUNK, on_light_done → unit-тики).
- Новая настройка [Scanner] scan_pool_width (default 200, min 4) — ширина пула в _run_powershell; юзер может снизить до 64/32.
- Моки test_scan38 probe_hosts переведены на (hosts, ping_ms, wmi_user=None); +2 теста (dc-first щадит машины; scan_pool_width). 413 passed + 2 skipped. Версия 3.9.8, тег v3.9.8; релиз 3.9.6 PATCHнут (имя/тело без имени продукта и без упоминания папок).

### Партия 59: полный опрос ускорен (3.9.7)
- Юзер (скриншот «Полный опрос: ПК 50/1648», жалоба «ооочень медленно» + вопрос про нагрузку на ПК пользователей). Причины: WMI-проход был на пуле 50 + FullScanWorker.CHUNK=50 → 1648/50=33 перезапуска powershell.exe. Сделано: pool по умолчанию 200 для ОБОИХ проходов (лёгкий и полный), CHUNK 50→200, WinRM -OperationTimeoutSec 3→2 (оба места каскада).
- Ответ юзеру про нагрузку: полный опрос трогает только ВКЛЮЧЕННЫЕ машины, по 1 пингу + 1–4 коротких WMI/WinRM/реестр-запроса (миллисекунды на машине); фоновый скан ПК пользователей не трогает вовсе (DNS-запрос к серверу + 1 ICMP-пакет).
- Версия 3.9.6 → 3.9.7, новый тег v3.9.7, релиз создан CI (softprops), имя/тело PATCHнуты.

### Партия 58: релиз 3.9.6 к стандарту остальных версий
- Юзер (скриншот страницы релиза): «приведи это к стандарту других изменений версии». Исправлено: имя релиза было «ADK v3.9.6» без описания → теперь с описанием, как у 3.9.4/3.9.5; тело — «## 3.9.6 — …», секции «### 🐞 Исправлено» (1–3) и «### 🛠 Улучшено (движок прошлого продукта)» (4–5, раньше была дыра в нумерации 5–6 после 1–3), «### 🧪 Тесты +4 … Всего 411». CHANGELOG в репо синхронно поправлен (заголовок секции, нумерация, формат тестов); коммит на main БЕЗ переноса тега (косметика, пересборка не нужна).

### Партия 57: CSV-зависимость убрана вовсе (3.9.6, переиздание 3)
- Юзер: «вообще убери зависимость от CSV — ADK должен сам уметь собирать и сопоставлять пк-юзер». Из _PS_SCANNER удалён весь блок чтения <PC>.csv (comp/compexit): переменные $compDir/$compExitDir, param-аргументы, AddArgument, подстановки __COMP_DIR__/__COMPEXIT_DIR__ в _run_powershell; LastLogon из сканера — всегда «Неизвестно» (реальную дату даёт журнал КД, база хранит прежнюю при «Неизвестно»).
- Источники «кто за ПК» теперь только свои: WMI-каскад (WinRM→DCOM→explorer.exe→реестр, полный опрос) + журнал КД 4768 (полный опрос, enrich_with_dc_logons). Фоновый проход юзера не трогает — batch_update_inventory хранит последнего известного (CASE WHEN excluded != '').
- Счётчики статуса: «машина X · CSV Y · КД Z» → «машина X · КД Z» (workers.scan_once, scan_ui шаги и full_summary_text); user_csv удалён из сводки.
- ОСТАВЛЕНО (сознательно): принтеры из invent_hardware_dir (СТОП-приказ юзера), характеристики описи (invent_hardware_dir), get_pc_info_from_csv в netutils — только fallback даты последнего входа для архивов «Прочитать всё» (login/fio → ip,дата); config-ключи comp/compexit остаются, комментарий в config.example обновлён.
- 411 passed + 2 skipped. Теги/релиз: v3.9.6 снова перенесён, релиз 394299833, тело PATCHнуто.

### Партия 56: откат сверки из прошлого продукта (3.9.6, переиздание 2)
- Юзер: «ADK сам создаёт базу, там где мы выбираем» (db_path в config.ini, pc_mapping.db) — сверку CSV по подсети из прошлого продукта копировать было не надо, нужна была только скорость. CSV-блок _PS_SCANNER возвращён к прежней логике ADK (Split(@(';',','))[1]); пул 200/SetMinThreads остаётся. Тест переписан (test_ps_scanner_csv_plain_login), 411 passed + 2 skipped.

### Партия 55: движок прошлого продукта в фоновом обходе (3.9.6, переиздание)
- Юзер передал ПОЛНЫЙ исходник своего прошлого продукта свой прошлый продукт (tkinter, .NET Async Engine) и приказал: фоновый обход и сверку позаимствовать у него (он быстрее), фильтрацию не трогать, «его тоже засунь в релиз 3.9.6», остальные фишки — потом в 3.9.7.
- Сохранён справочник: справочник прошлого продукта (удалён, партия 60) (в .gitignore, в git НЕ входит) — оттуда брать фишки 3.9.7: Ctrl+C копирование строки через запятую; сброс счётчиков дашборда при старте скана; бегущая строка статуса (TickerWidget); сортировка колонки IP как чисел; пользователь «—» для выключенных; AD-description как источник «кто за ПК» (внимание: ФИО из description ломает матчинг логинов в ADK — если брать, то отдельным полем/только для показа); строгие статусы «Активен (В сети)»/«Выключен».
