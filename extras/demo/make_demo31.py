...
ROOT = "/home/user"
OUT_DIR = "/home/user/extras/demo/frames31"
OUT_MP4 = "/home/user/extras/videos/ADK_demo_31.mp4"
FPS = 25
...
# ============================================================ 1b. 3.8.0: вопрос при старте + полный опрос
class _DemoFullScan(QThread):
    ...
    def run(self):
        ...
        self.step_text.emit("Шаг 1 из 3 — ПК из домена: адреса, кто за каким ПК, последние входы…")
        ...
        self.step_text.emit("Шаг 2 из 3 — принтеры с ПК в сети…")
        for d in range(61, online + 1, 61):
            _t.sleep(0.5)
            self.unit.emit("printers", d, f"WS-{200 + d}")
        self.step_text.emit("Шаг 3 из 3 — программы с ПК в сети…")
        for d in range(61, online + 1, 61):
            _t.sleep(0.5)
            self.unit.emit("software", d, f"WS-{200 + d}")
        self.finished_full.emit({"pcs": pcs, "online": online, "printers": 214, "printers_pcs": 198,
                                 "sw": 5210, "sw_pcs": 193, "mode": "full"})


cap("При запуске ADK больше не сканирует сам — спрашивает: всё, только ПК или ничего")
su = StartupScanDialog(w)
su.show(); wait(300)
S2 = S + [su]
snap(S2, 2)
hold(S2, 2.2)
_tiles = [b for b in su.findChildren(QPushButton) if b.findChildren(QLabel)]
_tile_full = next(b for b in _tiles if any("Всё" in l.text() for l in b.findChildren(QLabel)))
move_to(S2, _tile_full, 14)
hold(S2, 1.0)
press(S2, _tile_full)
su.choice = "full"; su.accept(); wait(250)
S2 = S
cap("«Всё» — три шага; круговая диаграмма показывает процент выполненных проверок")
_scan_ui.FullScanWorker, _orig_full_scan = _DemoFullScan, _scan_ui.FullScanWorker
fs = FullScanDialog(w.get_conn, "full", on_finished=lambda sm: w.lbl_status.setText(full_summary_text(sm)), parent=w)
fs.show(); wait(300)
S2 = S + [fs]
snap(S2, 2)
while fs.worker.isRunning():
    wait(120); snap(S2, sec(0.12))
wait(300)
hold(S2, 2.8)
move_to(S2, fs.btn_close, 10)
press(S2, fs.btn_close)
fs.accept(); wait(250)
