...
- **🧩 Расширяемость** — плагины (`Action`-класс → кнопка), CLI (`adk --find … --json`), серверный режим с PostgreSQL, ru/en, 10 тем (5 тёмных / 5 светлых) + свои цвета, единый набор контурных иконок.
...
```bash
pyflakes adk tests
QT_QPA_PLATFORM=offscreen pytest -q          # 309 тестов
```
...
- **3.6 (актуальная):** база без WAL (переносится на сервер), сторож зависшего запроса, резервные копии каждые 6 ч, один сканер на общую базу.
- **3.5:** автоопределение параметров домена AD, мастер «где база?» при первом запуске с автонаполнением, опрос парка без базы, справка по роли после входа, менеджер плагинов с шаблоном, 309 тестов.
